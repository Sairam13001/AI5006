...
%
\section{Problem}
If $\vec{A}$  = $\myvec{ \cos{\alpha} & -\sin{\alpha}\\ \sin{\alpha} & \cos{\alpha}}$, and $\vec{A}$ + $\vec{A}^\top$ = $\vec{I}$, then find the value of angle $\alpha$.


\section{Explanation}

The Complex number equivalent to the matrix $\myvec{a & -b\\ b & a }$ is $\myvec{a \\ b }$
