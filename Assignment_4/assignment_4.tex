...
\begin{figure}[!ht]
\centering
\resizebox{\columnwidth}{!}{\input{assignment_4_fig}}
...
\end{align}
Dividing equation \brak{\ref{innerprod1}} by \brak{\ref{innerprod2}}, we get : 
\begin{align}\label{innerproddiv}
     \frac{\cos{BAD}}{\cos{CAD}} = \frac{(\vec{A}-\vec{B})^T(\vec{A}-\vec{D})\norm{\vec{A}-\vec{C}}}{(\vec{A}-\vec{C})^T(\vec{A}-\vec{D})\norm{\vec{A}-\vec{B}}} 
\end{align}

Let $A = \myvec{0\\0}$, then equation \brak{\ref{innerproddiv}} becomes:
\begin{align}\label{innerproddiv2}
     \frac{\cos{BAD}}{\cos{CAD}} = \frac{(\vec{B})^T(\vec{D})\norm{\vec{A-C}}}{(\vec{C})^T(\vec{D})\norm{\vec{A-B}}} 
\end{align}

Using equation \brak{\ref{giveneqnedited}},  \brak{\ref{innerproddiv2}} can be written as : 
\begin{align}\label{innerproddiv3}
     \frac{\cos{BAD}}{\cos{CAD}} = \frac{(\vec{B})^T(\vec{D})\norm{\vec{C-D}}}{(\vec{C})^T(\vec{D})\norm{\vec{B-D}}} 
\end{align}

In a triangle that satisfies the equation \brak{\ref{givencond}}, It can be seen that the equation \brak{\ref{innerproddiv3}} always equates to 1, which implies that :
\begin{align}
